...
function deferPay(producer, amount, date) {
	var paymentDate = date.setDate(date.getDate() + producer['deferPeriod']);
	paymentDate = new Date(paymentDate);

 	deferedPayments.push({'producer': producer['name'],
 	'amount': amount,
...
console.log(`${deferedPayments[0]['paymentDate'].toLocaleDateString('ru-Ru')}: ${deferedPayments[0]['producer']}, сумма ${deferedPayments[0]['amount']} Q`);
console.log(`${deferedPayments[1]['paymentDate'].toLocaleDateString('ru-Ru')}: ${deferedPayments[1]['producer']}, сумма ${deferedPayments[1]['amount']} Q`);
console.log(`${deferedPayments[2]['paymentDate'].toLocaleDateString('ru-Ru')}: ${deferedPayments[2]['producer']}, сумма ${deferedPayments[2]['amount']} Q`);
...
if(loadCurrencyJSON()){
	try{
		var currencyExchange = JSON.parse(loadCurrencyJSON());
	}catch(err){
